...
export default function MovieDetails() {
  ...
  const hubcloudCacheRef = useRef<Record<string, { url: string, timestamp: number }>>({});
  ...
  const handlePlayClick = async (url: string, linkName?: string, linkId?: string, isZip?: boolean, tinyUrl?: string, isLocked?: boolean, seasonInfo?: { id: string; number: number; title?: string }) => {
    ...
    if (url.includes('hubcloud') || url.includes('moviesdrives')) {
      ...
      if (cached && (now - cached.timestamp < 30000)) {
         try {
            ...
            const checkRes = await fetch('/api/hubcloud/direct-link', { // Wait, do we have an api/check-link?
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ url: cached.url, checkOnly: true })
            }); // Let's create this checkOnly flag in /api/hubcloud/direct-link!
            if (checkRes.ok) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data.url && data.url !== url) {
               ...
               // Save to cache
               hubcloudCacheRef.current[url] = { url: finalUrl, timestamp: Date.now() };
            }
          }
        } catch (e) {
    ...
    setLinkPopup({ isOpen: true, url: finalUrl, name: linkName || 'Unknown Link', id: linkId || 'unknown', isZip, tinyUrl: finalTinyUrl });
  };
