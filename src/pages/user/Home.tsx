...
export default function Home({
  ...
  const showCatalogToast = (message: string, type: 'info' | 'success' = 'info') => {
  ...
  const renderSearchAndFiltersBlock = (isVisible: boolean, inputRef: React.RefObject<HTMLInputElement>) => (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          key="filters"
  ...
  const toggleTrendingRowVisibility = () => {
  ...
  const toggleNewlyAddedVisibility = () => {
...
{renderSearchAndFiltersBlock(showFilters, searchInputRef)}
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-zinc-200/80 dark:border-zinc-800/80">
              ...
              <button 
                ...
                title={isRecentVisible ? t('Collapse Recently Viewed') : t('Expand Recently Viewed')}
              >
                {isRecentVisible ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>
            </div>
        ...
        {!hideScrollingTabs && trendingCollection && (
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-zinc-200/80 dark:border-zinc-800/80">
              ...
              <button 
                onClick={toggleTrendingRowVisibility} 
                className="p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 dark:text-zinc-400 transition-colors"
                title={isTrendingRowVisible ? t('Collapse Trending Now') : t('Expand Trending Now')}
              >
                {isTrendingRowVisible ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>
            </div>
        ...
        {!hideScrollingTabs && newlyAddedCollection && (
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-zinc-200/80 dark:border-zinc-800/80">
              ...
              <button 
                onClick={toggleNewlyAddedVisibility} 
                className="p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 dark:text-zinc-400 transition-colors"
                title={isNewlyAddedVisible ? t('Collapse Newly Added') : t('Expand Newly Added')}
              >
                {isNewlyAddedVisible ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>
            </div>
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-zinc-200/80 dark:border-zinc-800/80">
              ...
              <button 
                ...
                className="p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 dark:text-zinc-400 transition-colors"
                title={isCollectionsVisible ? t('Collapse Collections') : t('Expand Collections')}
              >
                {isCollectionsVisible ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>
            </div>
        ...
        {renderSearchAndFiltersBlock(showCatalogFilters, catalogSearchInputRef)}
        {/* Grid */}
        {loading ? (
      ...
      <footer className="border-t border-zinc-200 dark:border-zinc-800 pt-6 pb-8 flex flex-col items-center gap-4">
        ...
        <div className="text-center text-xs text-zinc-500 dark:text-zinc-600 font-mono">
          {/* @ts-ignore */}
          v{typeof __APP_VERSION__ !== 'undefined' ? __APP_VERSION__ : '3.2.0'}
        </div>
      </footer>
