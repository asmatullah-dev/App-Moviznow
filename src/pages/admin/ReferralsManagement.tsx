...
export default function ReferralsManagement() {
  ...
  const exportCSV = () => {
    const headers = ['Inviter Email', 'Inviter Code', 'Referred User Email', 'Joined Date', 'Status', 'Signup Reward', 'Activation Reward'];
    const rows = [];

    inviters.forEach(inviter => {
      inviter.referredUsers.forEach((ref: any) => {
        rows.push([
          inviter.email || '',
          inviter.referralCode || '',
          ref.email || '',
          ref.createdAt ? format(new Date(ref.createdAt), 'yyyy-MM-dd') : '',
          ref.status || '',
          ref.signupRewardClaimed ? 'Yes' : 'No',
          ref.activationRewardClaimed ? 'Yes' : 'No'
        ]);
      });
    });
    ...
    link.setAttribute('download', `referrals_report_${format(new Date(), 'yyyy-MM-dd')}.csv`);
  ...
  const growthData = useMemo(() => {
    ...
    // Flatten all joins to get all referred users
    const allReferredUsers: any[] = [];
    referrals.forEach(ref => {
      if (ref.joins) {
        allReferredUsers.push(...Object.values(ref.joins));
      }
    });
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const count = allReferredUsers.filter(u => 
        u.createdAt && 
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          onClick={exportCSV}
          className="flex items-center justify-center gap-2 px-4 py-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-sm font-bold hover:scale-105 transition-transform"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h4 className="text-sm font-bold">Total Link Clicks</h4>
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Across all channels</p>
            </div>
          </div>
          <div className="text-3xl font-bold">{isLoadingAnalytics ? '...' : (analytics?.total_clicks || 0)}</div>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h4 className="text-sm font-bold">WhatsApp Clicks</h4>
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Most popular channel</p>
            </div>
          </div>
          <div className="text-3xl font-bold">{isLoadingAnalytics ? '...' : (analytics?.clicks_whatsapp || 0)}</div>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h4 className="text-sm font-bold">Telegram Clicks</h4>
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Secondary channel</p>
            </div>
          </div>
          <div className="text-3xl font-bold">{isLoadingAnalytics ? '...' : (analytics?.clicks_telegram || 0)}</div>
        </div>
      </div>
