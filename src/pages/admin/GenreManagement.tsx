...
export default function GenreManagement() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addAuxiliaryItem('genre', { name: newGenre.trim(), order: maxOrder + 1 });
      setNewGenre('');
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteAuxiliaryItem('genre', deleteId);
      setDeleteId(null);
    } finally {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await updateAuxiliaryItem('genre', editingId, { name: editName.trim() });
      setEditingId(null);
      setEditName('');
    } finally {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    await Promise.all(items.map((item, index) => 
      updateAuxiliaryItem('genre', item.id, { order: index })
    ));
  };
