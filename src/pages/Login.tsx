...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Only set to false if we are not successfully logged in (user will be set naturally if successful)
      // Actually we just wait for the AuthContext to change state and useEffect to redirect.
    } catch {
      setIsLoggingIn(false);
    }
  };

  // Ensure we reset logging state if there's an error from context
  useEffect(() => {
    if (error) {
      setIsLoggingIn(false);
    }
