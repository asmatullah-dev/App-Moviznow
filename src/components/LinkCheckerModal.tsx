...
const badgeMap: Record<StatusLabel, string> = {
  ...
  UNKNOWN:
    "bg-zinc-500/15 text-zinc-600 dark:text-zinc-300 border-zinc-300 dark:border-zinc-700",
...
export const LinkCheckerModal: React.FC<Props> = ({
  ...
  const [mdriveSelectedIndices, setMdriveSelectedIndices] = useState<
    Set<number>
  >(new Set());
  const [mdriveExtractingDirect, setMdriveExtractingDirect] = useState<
  ...
  const firstType = useMemo(
    () => (links[0] ? guessLinkType(links[0]) : "General link"),
    [links],
  );
  ...
  const updateBatchReviewItem = (
    key: string,
    field: "title" | "year",
    value: string,
  ) => {
    setBatchReviewItems((prev) =>
      prev.map((item) =>
  ...
  const handleMdriveSearch = async (targetUrl: string) => {
    ...
    try {
      const res = await fetch(
        `/api/mdrive?url=${encodeURIComponent(targetUrl)}`,
      );
      ...
      if (hits.length === 1) {
        ...
        console.log("MDrive auto-replacement:", {
          from: targetUrl,
          to: singleLink,
        });
        setInput(nextInput);
  ...
  const confirmMdriveSelection = () => {
    if (mdriveUrl && mdriveSelectedIndices.size > 0) {
      const selectedLinks = mdriveResults.filter((_, i) =>
      ...
      console.log("MDrive replacement:", {
        from: mdriveUrl,
        to: newLinksText,
        success: nextInput !== currentInput,
      });
      setInput(nextInput);
  ...
  const handleCheck = async (
    ...
    let currentLinks =
      onlyUrls ||
      splitLinks(currentInputSnapshot).map(normalizeUrl).filter(Boolean);
    ...
    const extractableLinks = currentLinks.filter(
      (u) =>
    ...
    if (extractableLinks.length > 0) {
      ...
      try {
        // Take up to 20 links at once
        const batch = extractableLinks.slice(0, 20);
          ...
          batch.map(async (targetUrl) => {
            try {
              if (targetUrl.includes("mdrive.lol")) {
                ...
                const res = await fetch(
                  `/api/moviesdrive?url=${encodeURIComponent(targetUrl)}`,
                );
                ...
                const res = await fetch(
                  `/api/filmygo?url=${encodeURIComponent(targetUrl)}`,
                );
        ...
        for (const res of results) {
          if (res.type === "auto") {
            ...
            if (res.extracted && res.extracted !== res.original) {
              const baseLink = res.original
                .replace(/^https?:\/\//, "")
                .replace(/\/$/, "");
              const escapedBase = baseLink.replace(
                /[.*+?^${}()|[\]\\]/g,
                "\\$&",
              );
              const regex = new RegExp(
                `(https?://)?(www\\.)?${escapedBase}/?`,
                "g",
              );
              nextInput = nextInput.replace(regex, res.extracted);
              console.log("Auto-replacement successful:", {
                from: res.original,
                to: res.extracted,
              });
            }
            ...
            res.type === "mdrive" ||
            res.type === "moviesdrive" ||
            res.type === "filmygo" ||
            res.type === "skymovieshd"
          ) {
            ...
            if (hits.length > 0) {
              if (res.type === "mdrive") {
                ...
                console.log(
                  `${res.type === "moviesdrive" ? "MoviesDrive" : res.type === "filmygo" ? "FilmyGo" : "SkymoviesHD"} auto-replacement successful:`,
                  { from: res.original, to: singleLink },
                );
              } else {
    ...
    const urls = currentLinks.filter(
      (u) =>
        !u.includes("mdrive.lol") &&
        !u.includes("howblogs.xyz") &&
        !u.includes("filesdl.in") &&
    ...
    try {
        ...
        while (queue.length > 0) {
          ...
          try {
            ...
            const errorResult: LinkCheckResult = {
              ...
              statusLabel: "UNKNOWN",
              message:
                e?.message || "Check failed due to a network or fetch error.",
            };
      ...
      const finalResults = allResults.map((r) => ({
        ...r,
        mismatchWarnings: buildMismatchWarnings(
          r,
          allResults,
          languages,
          qualities,
        ...
        confidenceScore: Math.max(
          0,
          100 -
            buildMismatchWarnings(r, allResults, languages, qualities).length *
              18,
        ),
      }));
        ...
        if (onlyUrls?.length) {
          ...
          return merged.map((r) => ({
            ...
            confidenceScore: Math.max(
              0,
              100 -
                buildMismatchWarnings(r, merged, languages, qualities).length *
                  18,
            ),
          }));
        }
  ...
  const handleAddLinks = () => {
    ...
    const qualityLinks: QualityLinks = validResults.map((r) => {
      ...
      const combinedMatch =
        source.match(/\bs(\d+)e(\d+)(?![a-z0-9])/i) ||
      ...
      if (combinedMatch) {
        ...
        const sMatch =
          source.match(/\bs(\d+)\b/i) ||
          source.match(/season\s*(\d+)/i) ||
          source.match(/ss\s*(\d+)/i);
        const eMatch =
          source.match(/\be(\d+)\b/i) ||
          source.match(/episode\s*(\d+)\b/i) ||
          source.match(/ep\s*(\d+)\b/i);
      ...
      if (r.fileSizeText) {
        ...
        if (
          parts.length === 2 &&
          (parts[1].toUpperCase() === "MB" || parts[1].toUpperCase() === "GB")
        ) {
          sizeStr = parts[0];
      ...
      else if (/full season|all episodes|complete/i.test(source) && !detectedE)
    ...
    if (isBatchMode && onBatchAddLinks) {
      ...
      qualityLinks.forEach((ql, idx) => {
        ...
        const { title: extractedTitle, year: extractedYear } =
          extractTitleAndYear(sourceText);
        ...
        if (
          ...
          ql.isFullSeasonMKV ||
          ql.isFullSeasonZIP ||
          /full season|all episodes|complete/i.test(sourceText)
        ) {
        ...
        if (r.audioLabel) {
          r.audioLabel
            .split(" / ")
            .forEach((l) => batch.detectMetadata.languages.add(l));
        }
        ...
        if (
          r.subtitleLabel ||
          /subtitles|subs|softsub|hardsub|esub|esubs|msub|msubs/i.test(source)
        ) {
          batch.detectMetadata.subtitles = true;
        }

        // Apply detected S/E to batch metadata if not already set (fallback for creation)
        if (ql.season && !batch.detectMetadata.season)
          batch.detectMetadata.season = ql.season;
        if (ql.episode && !batch.detectMetadata.episode)
          batch.detectMetadata.episode = ql.episode;
      });

      const itemsToReview = Array.from(batchesMap.entries()).map(
        ([key, b]) => ({
          key,
          title: b.title,
    ...
    validResults.forEach((r) => {
      ...
      if (
        r.isFullSeasonMKV ||
        r.isFullSeasonZIP ||
        /full season|all episodes|complete/i.test(source)
      ) {
        isSeriesLink = true;
      }
    ...
    if (
      ...
      validResults.some(
        (r) =>
          r.isFullSeasonMKV ||
          r.isFullSeasonZIP ||
          (r.season === 1 && r.episode === 1) ||
          /full season|all episodes|complete/i.test(
            `${r.fileName || ""} ${r.finalUrl || ""}`,
          ),
      )
    ) {
      detectedType = "series";
    ...
    const combinedNames =
      validResults.map((r) => r.fileName || "").join(" ") + " " + input;
    const { title: extractedTitle, year: extractedYear } =
      extractTitleAndYear(combinedNames);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (
        ...
        const initialLinks = splitLinks(initialInput)
          .map(normalizeUrl)
          .filter(Boolean);
  ...
  const retryFailed = () => {
    const failed = results
      .filter(
        (r) =>
          !r.ok ||
          r.statusLabel === "UNKNOWN" ||
          r.statusLabel === "MISSING_FILENAME" ||
          r.statusLabel === "BROKEN" ||
  ...
  const summary = useMemo(() => {
    ...
    const protectedCount = results.filter(
      (r) => r.statusLabel === "PROTECTED",
    ).length;
    const redirect = results.filter((r) => r.statusLabel === "REDIRECT").length;
    const unavailable = results.filter(
      (r) => r.statusLabel === "UNAVAILABLE",
    ).length;
    const unknown = results.filter((r) => r.statusLabel === "UNKNOWN").length;
    const mismatches = results.filter(
      (r) => (r.mismatchWarnings?.length || 0) > 0,
    ).length;
    const missingFilename = results.filter(
      (r) => r.statusLabel === "MISSING_FILENAME",
    ).length;
    const missingQuality = results.filter(
      (r) =>
        r.statusLabel === "MISSING_METADATA" && r.message?.includes("Quality"),
    ).length;
    const missingLanguage = results.filter(
      (r) =>
        r.statusLabel === "MISSING_METADATA" && r.message?.includes("Language"),
    ).length;
    const smallFile = results.filter(
      (r) => r.statusLabel === "SMALL_FILE",
    ).length;
    const sizeMismatch = results.filter(
      (r) => r.statusLabel === "SIZE_MISMATCH",
    ).length;
    return {
      working,
      broken,
      ...
      {isOpen ? (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-5xl max-h-[95vh] overflow-y-auto custom-scrollbar"
          >
            <div className="rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 shadow-2xl overflow-hidden transition-colors duration-300">
              <div className="p-5 md:p-6 space-y-5">
                    ...
                    <div>
                      <h2 className="text-xl font-semibold leading-none text-zinc-900 dark:text-white">
                        {isReviewingBatch
                          ? "Review Batch Items"
                          : isBatchMode
                            ? "Batch Link Checker (Missing Details)"
                            : title}
                      </h2>
                      <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                        {isReviewingBatch
                          ? "Please verify and enter missing years for each item before saving."
                          : "Check Pixeldrain, direct file links, protected download gateways, and missing movie posts."}
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={handleClose}
                    className="rounded-full px-3 py-1.5 text-sm text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white transition"
                  >
                    Close
                  </button>
                </div>
                    ...
                    <div className="flex items-center justify-between px-1">
                      <div>
                        <h3 className="text-lg font-bold">
                          {mdriveUrl.includes("mdrive.lol")
                            ? "MDrive Selection"
                            : mdriveUrl.includes("filmygo.")
                              ? "FilmyGo FilesDL Selection"
                              : mdriveUrl.includes("skymovieshd.")
                                ? "SkymoviesHD HowBlogs Selection"
                                : "MoviesDrive MDrive Selection"}
                        </h3>
                        <p className="text-xs text-zinc-500">
                          {mdriveUrl.includes("mdrive.lol")
                            ? "Pick the Hubcloud links you want to extract"
                            : mdriveUrl.includes("filmygo.")
                              ? "Pick the FilesDL links you want to process"
                              : mdriveUrl.includes("skymovieshd.")
                                ? "Pick the HowBlogs links you want to process"
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <button
                          onClick={() => {
                            if (
                              mdriveResults.length > 0 &&
                              mdriveSelectedIndices.size ===
                                mdriveResults.length
                            ) {
                              setMdriveSelectedIndices(new Set());
                            } else {
                              setMdriveSelectedIndices(
                                new Set(mdriveResults.keys()),
                              );
                            }
                          }}
                          className={`text-xs font-bold px-3 py-1 rounded-lg transition-colors ${
                            mdriveResults.length > 0 &&
                            mdriveSelectedIndices.size === mdriveResults.length
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {!item.is_direct && (
                                ...
                                >
                                  {mdriveExtractingDirect[i] ? (
                                    <LoaderIcon className="w-4 h-4 animate-spin text-cyan-500" />
                                  ) : (
                              ...
                              {item.is_direct && (
                                <div
                                  className="p-2 bg-cyan-500/20 rounded-xl text-cyan-500"
                                  title="Direct Drive Link Extracted"
                                >
                        ...
                        >
                          <div className="flex-1 space-y-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                              <span
                                className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${item.metadata.type === "series" ? "bg-indigo-500/15 text-indigo-500 border-indigo-500/30" : "bg-blue-500/15 text-blue-500 border-blue-500/30"}`}
                              >
                                {item.metadata.type}
                              </span>
                  ...
                  <>
                    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/60 dark:bg-zinc-900/60 p-4 space-y-3 transition-colors duration-300">
                      <label className="text-sm font-medium text-zinc-700 dark:text-zinc-200">
                        Paste one or multiple links / full movie post
                      </label>
                      <textarea
                        ref={textareaRef}
                        value={input}
                        onChange={(e) => setInput(e.target.value)}
                        placeholder="Paste links or a full movie post here..."
                        rows={6}
                        className="w-full rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-4 py-3 text-sm text-zinc-900 dark:text-white placeholder:text-zinc-500 outline-none focus:ring-2 focus:ring-cyan-500 transition-colors duration-300"
                      />
                        ...
                        <div className="flex flex-col gap-2">
                          <label className="inline-flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-300 cursor-pointer select-none">
                            <input
                              ...
                              onChange={(e) =>
                                setAutoClipboard(e.target.checked)
                              }
                            ...
                            />
                            Auto-detect and paste links from clipboard (Every
                            3s)
                          </label>
                          {autoClipboard && (
                            <div className="text-xs pl-6 transition-all duration-300">
                              {clipboardStatus === "denied" ? (
                                <span className="text-rose-600 dark:text-rose-400 font-medium flex items-center gap-1.5 animate-fade-in">
                                  ...
                                  Access restricted (Please grant clipboard
                                  permission)
                                </span>
                              ) : (
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          ...
                          className="inline-flex items-center justify-center rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-950 px-8 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 gap-2 transition-colors w-32"
                        >
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-zinc-500 dark:text-zinc-400">
                        ...
                        <span className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                          ...
                          Checks only when manually used
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        onClick={() => handleCheck()}
                        disabled={loading}
                        className="inline-flex items-center justify-center rounded-2xl gap-2 bg-cyan-500 px-4 py-2 text-sm font-bold text-white hover:bg-cyan-600 dark:hover:bg-cyan-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                      >
                        {loading ? (
                        ...
                        {loading
                          ? "Checking..."
                          : `Check ${links.length || ""} Link${links.length > 1 ? "s" : ""}`}
                      </button>
                      <button
                        onClick={retryFailed}
                        className="inline-flex items-center justify-center rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-transparent px-4 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 gap-2 disabled:opacity-50 transition-colors"
                        disabled={
                          loading ||
                          !results.some(
                            (r) =>
                      ...
                      <button
                        onClick={reset}
                        className="inline-flex items-center justify-center rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-transparent px-4 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 gap-2 transition-colors"
                      >
                      ...
                      {(onAddLinks || onBatchAddLinks) &&
                        selectedUrls.size > 0 &&
                        !loading && (
                          <button
                            onClick={handleAddLinks}
                            className="inline-flex items-center justify-center rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white dark:text-black hover:bg-emerald-700 gap-2 ml-auto transition-colors"
                          >
                            <LinkIcon className="h-4 w-4" />
                            {isBatchMode
                              ? `Add ${selectedUrls.size} Links Missing`
                ...
                {error ? (
                  <div className="rounded-2xl border border-red-200 dark:border-red-900/70 bg-red-50 dark:bg-red-950/40 p-4 text-red-600 dark:text-red-300 text-sm flex items-start gap-2 transition-colors duration-300">
                ...
                {!!results.length && (
                  <div className="grid grid-cols-2 md:grid-cols-9 gap-3">
                    {[
                      ...
                      <div
                        key={String(label)}
                        className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/60 dark:bg-zinc-900/60 p-4 transition-colors duration-300"
                      >
                        <div className={`text-sm ${color}`}>{label}</div>
                        <div className="text-2xl font-semibold text-zinc-900 dark:text-white mt-1">
                          {count}
                        </div>
                      </div>
                    ))}
                ...
                <div className="space-y-3 max-h-[500px] overflow-auto pr-1">
                  {sortedResults.map((result) => {
                    const statusLabel =
                      result.statusLabel || (result.ok ? "WORKING" : "UNKNOWN");
                    ...
                    if (
                      result.audioLabel &&
                      result.audioLabel.includes("Dual") &&
                      result.codecLabel !== "HEVC"
                    )
                      finalName += " Dual";

                    return (
                      <div
                        key={`${result.url}-${result.qualityLabel || "na"}`}
                        className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 overflow-hidden transition-colors duration-300"
                      >
                        <div className="p-4 space-y-3">
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                            <div className="min-w-0 flex-1 flex items-start gap-3">
                              <div className="mt-1">
                                <input
                                  ...
                                  className="h-5 w-5 rounded border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950"
                                />
                              </div>
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  <div
                                    className={`inline-flex rounded-full border px-3 py-1 text-xs font-medium ${badgeMap[statusLabel]}`}
                                  >
                                    {statusLabel}
                                  </div>
                                  {result.ok && (
                                  ...
                                  {result.isDirectDownload ? (
                                    <div className="inline-flex rounded-full border border-blue-200 dark:border-blue-800 bg-blue-500/10 px-3 py-1 text-xs font-medium text-blue-600 dark:text-blue-400">
                                  ...
                                  0 ? (
                                    <div className="inline-flex rounded-full border border-pink-200 dark:border-pink-800 bg-pink-500/10 px-3 py-1 text-xs font-medium text-pink-600 dark:text-pink-400">
                                ...
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {result.qualityLabel ? (
                                    <span className="rounded-full border border-fuchsia-200 dark:border-fuchsia-800 bg-fuchsia-500/10 px-2.5 py-1 text-[11px] font-medium text-fuchsia-600 dark:text-fuchsia-300">
                                      {result.qualityLabel}
                                    </span>
                                  ) : null}
                                  {result.printQualityLabel ? (
                                    <span className="rounded-full border border-rose-200 dark:border-rose-800 bg-rose-500/10 px-2.5 py-1 text-[11px] font-medium text-rose-600 dark:text-rose-300">
                                      {result.printQualityLabel}
                                    </span>
                                  ) : null}
                                  {result.codecLabel ? (
                                    <span className="rounded-full border border-indigo-200 dark:border-indigo-800 bg-indigo-500/10 px-2.5 py-1 text-[11px] font-medium text-indigo-600 dark:text-indigo-300">
                                      {result.codecLabel}
                                    </span>
                                  ) : null}
                                  {result.audioLabel ? (
                                    <span className="rounded-full border border-emerald-200 dark:border-emerald-800 bg-emerald-500/10 px-2.5 py-1 text-[11px] font-medium text-emerald-600 dark:text-emerald-300">
                                      {result.audioLabel}
                                    </span>
                                  ) : null}
                                  {result.subtitleLabel ? (
                                    <span className="rounded-full border border-amber-200 dark:border-amber-800 bg-amber-500/10 px-2.5 py-1 text-[11px] font-medium text-amber-600 dark:text-amber-300">
                                      {result.subtitleLabel}
                                    </span>
                                  ) : null}
                                  {result.season ? (
                                    <span className="rounded-full border border-blue-200 dark:border-blue-800 bg-blue-500/10 px-2.5 py-1 text-[11px] font-bold text-blue-600 dark:text-blue-300">
                                      Season {result.season}
                                    </span>
                                  ) : null}
                                  {result.episode ? (
                                    <span className="rounded-full border border-indigo-200 dark:border-indigo-800 bg-indigo-500/10 px-2.5 py-1 text-[11px] font-bold text-indigo-600 dark:text-indigo-300">
                                      Episode {result.episode}
                                    </span>
                                  ) : null}
                                  {result.isFullSeasonMKV ? (
                                    <span className="rounded-full border border-purple-200 dark:border-purple-800 bg-purple-500/10 px-2.5 py-1 text-[11px] font-bold text-purple-600 dark:text-purple-300">
                                      Full Season MKV
                                    </span>
                                  ) : null}
                                  {result.isFullSeasonZIP ? (
                                    <span className="rounded-full border border-purple-200 dark:border-purple-800 bg-purple-500/10 px-2.5 py-1 text-[11px] font-bold text-purple-600 dark:text-purple-300">
                                      Full Season ZIP
                                    </span>
                                  ) : null}
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-2 items-center">
                                      ...
                                      {result.candidates.map((cand, idx) => {
                                        ...
                                        return (
                                          <span
                                            key={idx}
                                            className="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-200/50 dark:bg-zinc-800/50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-zinc-600 dark:text-zinc-300"
                                          >
                                            {name}
                                          </span>
                                        );
                            ...
                            <div className="flex gap-2 self-start">
                              {(!result.ok ||
                                ...
                                result.statusLabel === "UNAVAILABLE") && (
                                <button
                          ...
                          {openRow ? (
                            <div className="grid gap-2 text-xs text-zinc-500 dark:text-zinc-400 sm:grid-cols-2 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950/70 p-4 transition-colors duration-300">
                              ...
                              {result.source ? (
                                <div>Method: {result.source}</div>
                              ) : null}
                              {result.fileName ? (
                                <div>File Name: {result.fileName}</div>
                              ) : null}
                              {result.fileSizeText ? (
                                <div>File Size: {result.fileSizeText}</div>
                              ) : null}
                              {result.qualityLabel ? (
                                <div>Quality: {result.qualityLabel}</div>
                              ) : null}
                              {result.printQualityLabel ? (
                                <div>
                                  Print Quality: {result.printQualityLabel}
                                </div>
                              ) : null}
                              {result.codecLabel ? (
                                <div>Codec: {result.codecLabel}</div>
                              ) : null}
                              {result.audioLabel ? (
                                <div>Audio: {result.audioLabel}</div>
                              ) : null}
                              {result.subtitleLabel ? (
                                <div>Subtitles: {result.subtitleLabel}</div>
                              ) : null}
                              {result.season ? (
                                <div>Season: {result.season}</div>
                              ) : null}
                              {result.episode ? (
                                <div>Episode: {result.episode}</div>
                              ) : null}
                              {result.isFullSeasonMKV ? (
                                <div>Full Season MKV: Yes</div>
                              ) : null}
                              {result.isFullSeasonZIP ? (
                                <div>Full Season ZIP: Yes</div>
                              ) : null}
                              {typeof result.confidenceScore === "number" ? (
                                <div>Confidence: {result.confidenceScore}%</div>
                              ) : null}
                              {result.finalUrl ? (
                                <div className="sm:col-span-2 break-all text-zinc-600 dark:text-zinc-300">
                              ...
                              {(result.mismatchWarnings?.length || 0) > 0 ? (
                                <div className="sm:col-span-2 rounded-xl border border-pink-200 dark:border-pink-900/70 bg-pink-50 dark:bg-pink-950/30 p-3 text-pink-600 dark:text-pink-300 transition-colors duration-300">
                                  ...
                                  <ul className="list-disc pl-5 space-y-1">
                                    {result.mismatchWarnings?.map((w, i) => (
                                      <li key={i}>{w}</li>
                                    ))}
                                  </ul>
                                </div>
