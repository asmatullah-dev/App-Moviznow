...
const QualityInputs: React.FC<QualityInputsProps> = ({ links, onChange, droppableId }) => {
  ...
  const handleUrlBlur = async (url: string, idx: number) => {
    ...
    if (url.includes('hubcloud') || url.includes('vcloud') || url.includes('hubdrive') || url.includes('moviesdrive') || url.includes('skymovies') || url.includes('mdrive') || url.includes('filmygo')) {
      try {
