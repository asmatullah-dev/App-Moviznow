...
const htmlCache = new Map<
  string,
  { data: any; status: number; headers: any; timestamp: number }
>();
  ...
  try {
    ...
    if (checkOnly && url) {
      try {
        ...
        if (
          (checkRes.status === 403 || checkRes.status === 503) &&
          (isVcloud || url.includes("vcloud"))
        ) {
    ...
    candidateLinks.sort((a, b) => {
      ...
      const isA_FSL =
        /fsl|servers*2/i.test(a.text) || /fsl|servers*2/i.test(a.href);
      const isB_FSL =
        /fsl|servers*2/i.test(b.text) || /fsl|servers*2/i.test(b.href);
    ...
    for (const c of returnCandidates) {
      if (
        c.href.includes("hubcloud") ||
        c.href.includes("moviesdrive") ||
        c.href.includes("vcloud") ||
        c.href.includes("hubdrive")
      ) {
    ...
    if (nextHubcloudLink && nextHubcloudLink !== url) {
      try {
        const recursiveRes = await performExtraction(
          nextHubcloudLink,
          false,
          depth + 1,
        );
    ...
    if (
      (e.isAxiosError && e.message.includes("maxContentLength exceed")) ||
      ...
      if (
        ...
        !finalUrl.includes("moviesdrive") &&
        !finalUrl.includes("vcloud") &&
        !finalUrl.includes("hubdrive")
      ) {
